...
> **Статус: ревизия 2 (после внешнего ревью), аннотация 2026-08-20.** Исправлены R1, R2, R3, R4, R5, R6, проверка chain ID и раздел расхождений с документацией; добавлен раздел «Находки» и систематическое доказательство для R8. Creditcoin Team предварительно ответила, что `verifyAndEmit`, вероятно, должен тарифицировать лог; окончательное решение ожидается. Журнал изменений — в конце файла.
...
| R2. `verify`/`verifyAndEmit` | На закреплённом пине сигнатуры, возврат и событие подтверждены. Логика проверки и её ручные gas-charges идентичны; `verifyAndEmit` дополнительно исполняет `LOG3` и `calculate_tx_index_impl`, **но не тарифицирует ни то, ни другое** — см. «Находки», F-1. Creditcoin Team предварительно считает, что лог должен тарифицироваться; будущее поведение открыто. | Подтверждено кодом на пине; изменение тарификации ожидает ответа команды |
...
### F-1. На проверенном пине `verifyAndEmit` не тарифицирует стоимость EVM-лога
...
**Живая проверка и внешний статус.** На одинаковых входах Creditcoin3 Testnet
`eth_estimateGas` дал 42 966 для обеих функций, расхождение 0; методика и данные —
в `docs/ATTESTCOIN_INTEGRATION.md`. Creditcoin Team 2026-08-20 предварительно
ответила, что лог технически должен тарифицироваться и gas recording может вырасти
примерно на стоимость `LOG3`; окончательный ответ ожидается. Поэтому F-1 остаётся
точным описанием закреплённого пина и текущего измерения, но не прогнозом будущей
тарификации.
...
7. **Будущая тарификация F-1** — Creditcoin Team предварительно считает, что лог должен тарифицироваться; окончательный ответ ожидается.
