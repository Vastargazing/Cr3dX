...
> **This file is in Russian, and it is not the entry point.** It is the complete
> chronological source journal: every session, decision, measurement and live
> observation, newest entry first. It is kept in the language it was written in
> because rewriting a journal after the fact would destroy its value as
> provenance. English readers should start at
> [`STATUS_EN.md`](STATUS_EN.md) for the current state, the
> [English v0.4.8 specification](CR3DX_SPEC_V0.4.8_EN.md) for behavior, and the
> repository [README](../README.md) for the project. Come back here only when
> you need the exact history behind a specific claim.
...
Файл длинный и растёт: последняя запись сверху, актуальное состояние — в верхних
двух-трёх записях. Оглавление: `grep -n "^## " docs/STATUS.md`. Записи ниже —
история и свидетельства, не текущий список задач.

---

## 2026-08-24 — предсабмитный редакторский проход

Редакторский проход перед сабмитом. Поведение не менялось: ни контрактов, ни
тестов, ни деплоя. Последний поведенческий коммит на `main` остаётся
`229d084efd898e16526699da8988de88af9bf1b5`.

**Проверено машинно, до правок.**

- Вся история репозитория просканирована на ключевой материал: каждый блоб
  каждой достижимой ревизии, 376 значений вида `0x`+64 hex приведены к адресу
  через `ethers.Wallet`; ни одно не даёт адрес A или B. Совпадений по
  mnemonic/seed, PEM, AWS/GitHub/Slack-токенам, webhook и API-ключам в
  endpoint нет. `.env` в истории не было ни разу, только `.env.example`.
- Все относительные ссылки во всех tracked `*.md` и `ui/index.html`
  разрешаются: битых нет. Все внешние ссылки отвечают 200, кроме двух, которые
  и должны падать сейчас: сам репозиторий и GitHub Pages.
- Четыре транзакции с деки живы на Creditcoin, статус `ok`, адресат —
  текущий `Cr3dXDeals`, `gas_used` совпадает с цифрами слайда 10 до единицы:
  260 288, 340 382, 292 348, 266 056.
- Живое состояние кредитного слоя сходится со слайдом 4: score 525, лимит
  5 250 USDC, exposure 0, reserved 0, одна сделка.
- Все печати `docs/verification/v0.4.8-phase-b/` проверяются: payload-ledger
  фазы A (11 файлов), самопечать freeze-manifest, печать B0 (4 файла). Все 13
  якорей таблицы в README чекпоинта совпали с файлами на диске.
- Полный базовый прогон: Foundry 141/141 (9 сюит, 8 инвариантных/property),
  `test:scripts` 89 тестов в 10 файловых сюитах, `typecheck`, `ui:typecheck`,
  `test:ui` 15/15 — всё зелёное.

**Исправлено.**

1. Дека: `docProps` содержали `Walnut Exporter` в `dc:creator`,
   `lastModifiedBy` и `ap:Application`, а `dc:title` был `Presentation`.
   Заменено на автора и название проекта. Ссылка слайда 4 и заметки
   докладчика к слайдам 4, 7 и 10 вели на русский `docs/STATUS.md` — судья
   попадал в 2155 строк русского текста; переведены на `docs/STATUS_EN.md`.
   Пересобрано так, что список записей zip идентичен исходному, а содержимое
   отличается ровно в шести файлах.
2. Остатки русского в английских документах: пять фрагментов в
   `PRECOMPILE_FINDINGS.md` (комментарии внутри цитат кода и строка
   арифметики), там же `1 756` и `17 560` приведены к `1,756` и `17,560`.
   Фраза авторизации `РАЗРЕШАЮ S6 LIVE` оставлена как есть — она сверяется
   буквально, — но теперь в обоих документах объяснено, почему она на языке
   оператора. Русский заголовок раздела STATUS в
   `audit/attestcoin-integration-summary.md` оставлен как точная ссылка и
   снабжён английским переводом.
3. `audit/repo-cleanup-report.md` называл конкретные инструментальные
   директории локальной среды. Названия убраны, вывод отчёта не изменился;
   редакция явно зафиксирована в `audit/README.md`, а не сделана молча.
4. Два русских документа получили английскую врезку в первой строке:
   `STATUS.md` и `cr3dx-spec-v0.4.0-final.md`. Оба остаются на русском, но
   читатель сразу видит, что это провенанс, и куда идти за английским.
5. README: добавлен абзац о том, что цифры S5 сняты на предыдущем реестре, а
   текущие адреса отвечают состоянием S6, со ссылкой на `previousDeals`. Без
   него судья, сверяющий адреса, видел 525 вместо 550 и считал это
   расхождением.
6. `STATUS_EN.md`: снимок был приколот к `229d084` датой 2026-08-22; теперь
   явно сказано, что это последний поведенческий коммит, а всё после него —
   документация и подача.
7. README чекпоинта верификации: добавлены команды проверки печатей и раздел
   о том, почему запечатанные манифесты не переписываются под общий стиль.
   Они хранят абсолютные пути изолированного рабочего пространства модели, и
   эти пути входят в хеши: переписать их значит сломать печати, а пересчитать
   печати значит подменить свидетельство, запечатанное до доступа к
   реализации, свидетельством после. Файлы остаются байт-в-байт.
8. Добавлен `LICENSE` (MIT) — SPDX-заголовки в контрактах его объявляли, а
   файла в репозитории не было.

**Осталось координатору, вне этого дерева.** Репозиторий приватный, GitHub
Pages не включены; обе ссылки деки на них сейчас 404. Атрибуция инструментов
осталась в метаданных истории: имена веток `codex/*` в трёх merge-коммитах и
`feat/w1-network-probe` в одном, а также строка `CLAUDE.md` в diff `.gitignore`
двух коммитов. Переписывать историю ради этого не предлагается: это сменит
хеши, включая `cbc382b` и `1a30881`, на которых держится вся цепочка Phase B.

---

## 2026-08-22 — дайджест спецификации независимо сверен

Чат спецификации сверил `docs/SPEC_DIGEST_EN.md` с русской v0.4.8 и нашёл четыре
узких места на границе между спекой, реализацией и пересказом; контрактных
дефектов не найдено. Каждое проверено по тексту спеки до применения:

1. `abi.encode` у `termsHash` и `dealId` — форма реализации; спека (3.1) задаёт
   только последовательность полей. Для `evidenceId` точная форма
   `keccak256(abi.encode(chainKey, height, txIndex, uint8(kind), eventNonce))`
   зафиксирована самой спекой (4.2). Дайджест теперь различает эти случаи.
2. Фандинг просроченной сделки: «немедленно допускает дефолт» заменено правилом
   из 4.3 — сразу, если аттестованная высота уже выше `dueBlock + grace`, иначе
   при достижении порога. Спека (3.4) говорит «кандидатом на дефолт».
3. «Complete state-changing surface» сужено до permissionless-поверхности на
   Creditcoin (`Verifier.submitEvidence*`, `Deals.createDeal`, `submitAndApply*`,
   `applyEvidence`, `markDefaulted`); `Credit` пишется только своим `Deals`.
   Уточнение к предложенной формулировке: на Sepolia поверхность — `Gateway.fund`
   и `Gateway.repay` (4.1), это добавлено явно.
4. В учётной таблице «any funding» → «any applied funding». Таблица спеки (4.3,
   «любой фандинг») мирится с этим по контексту применения; дайджест точнее.

Плюс шапка: «introduces no normative rule; current-deployment values and
implementation-only names are labelled explicitly». Doc-only, `contracts/` и
тесты не тронуты, `git diff --check` и проверка ссылок чистые. С этого момента
дайджест считается сверенным независимой стороной; новый широкий раунд не нужен.

---

## 2026-08-22 — полировка после PR #7: сверка дайджеста, быстрая петля, типы live-скрипта

Продолжение редакционного прохода. Поведение контрактов, спецификация, deployment
ledger и запечатанные артефакты не менялись; `contracts/` не тронут.

**Сверка `docs/SPEC_DIGEST_EN.md` со спецификацией.** Построчная проверка терминов
и чисел дайджеста против разделов 3.3, 3.4, 4.3, 4.4 и 5 нашла три имени, которые
дайджест подавал как спецификационные, хотя они существуют только в реализации:
`DefaultTooEarly`, `SourceTransactionFailed` (спека требует проверку `status == 1`,
но ошибку не именует) и `scoreFromOutcomes` (спека формулирует INV-5, а
on-chain пересчёт — свойство реализации). Все три теперь помечены как имена
реализации. Формулировка о стороннем плательщике приведена к тому, что спека
действительно говорит: перечень проверок погашения плательщика не содержит.
Числа (grace 600, `BASE_LIMIT` 5 000 000 000, веса +25/−50/−200, кламп 300…850,
потолок батча 10) совпали. Дайджест остаётся ненормативным; независимая
рецензия через процесс спецификации ещё не проводилась.

**Быстрая петля.** В `package.json` добавлены `test:quick` (Foundry без stateful
invariant-файла) и `check:quick` (`test:quick` + `test:scripts` + `typecheck`).
Измерено: 13 с на тёплом кэше компилятора, около минуты с холодной компиляцией.
README ссылается на неё вместо голой команды `forge`; полный трёхкомандный
baseline перед handoff остаётся обязательным. В шапку этого файла добавлена
навигационная подсказка (последняя запись сверху, оглавление через `grep`).

**Типы в `scripts/deal-live.ts`.** `any` в сигнатурах `Context`, `recordCost`,
`send`, `sendKnownTransaction`, `parseEvents` заменён на `Wallet`,
`TransactionReceipt`, `TransactionRequest` и `Result` из ethers v6. Единственное
нетиповое следствие: `feeOf` с запасной веткой `effectiveGasPrice` заменён прямым
`receipt.fee` — в ethers v6 это геттер `gasUsed * gasPrice`, то есть та ветка,
которая всегда и срабатывала. Логика отправки, ретраев и проверок не менялась;
проверка — `tsc --noEmit`, живой запуск не выполнялся. `any` в `capture-*`,
`deploy-*`, `verify-live` и `probe` оставлен: это одноразовые инструменты измерения
и деплоя, не демо-путь.

**Проверки без находок.** `npm audit` (с dev и без): 0 уязвимостей. Все
относительные ссылки в README, дайджесте, обоих ранбуках и `ui/README.md`
разрешаются в существующие файлы. Повторное адверсариальное чтение четырёх
production-контрактов (порядок CEI в гейте, реентерабельность через токен,
уникальность `evidenceId`, пересечение порога, насыщение, счётчики `uint32`,
`markDefaulted` из чужого статуса, самофинансирование) новых находок не дало;
самофинансирование остаётся заявленной границей доверия, а не дефектом. Это
не внешний аудит и не заменяет его. Статические анализаторы Solidity в окружении
не установлены.

**Не сделано намеренно.** Двойное чтение `getEvidence` в двух фазах и дубль
константы `STATUS_SUCCESS` — изменения байткода, обесценивающие печать фазы B.
Строка 107 в `docs/audit/repo-cleanup-report.md` не редактировалась: это
исторический аудит, запись о том, что было удалено, является provenance.

---

## 2026-08-22 — source-only clarification после фазы B, английский дайджест, пин адресов

Редакционный проход после второй независимой проверки onboarding. Поведение не
менялось, спецификация не менялась, передеплоя не было. Замороженные хеши
остаются прежними и этим проходом не затронуты: spec-коммит v0.4.8
`cbc382b39fabd9a34b218fe6ff35699e18bdca4a`, implementation-коммит
`1a308816ab3b73056718f6f174c47c10f9fb8cd3`, SHA-256 нормативного пакета
`c8119bb3b8aba49348bc467ccb085bf4ad4afc98781463e3c644d091b12c7b80`, все манифесты
и печати в `docs/verification/v0.4.8-phase-b/` (ни один из них не хеширует
исходники `contracts/`; манифест B1 перечисляет только `Replay_*.t.sol`).

**Source-only clarification в `contracts/interfaces/IBlockProver.sol`.** Комментарий
к событию `TransactionVerified` утверждал, что эмиссия «ничего не стоит» и бюджет
на неё закладывать не нужно. Это противоречило разделу 9 журнала v0.4.8 и находке
F-1 в `docs/ATTESTCOIN_INTEGRATION.md`: измерение 2026-08-19 дало ноль, но
Creditcoin Team 2026-08-20 предварительно считает, что `LOG3` должен
тарифицироваться, и бюджет обязан терпеть рост примерно на 1756 газа за событие.
Комментарий приведён к этой формулировке; изменены только строки комментария
(7 добавлено, 3 удалено), код не тронут.

Проверка байткода выполнена до и после правки чистой сборкой `forge build --force`
(`bytecode_hash = "none"`, `cbor_metadata = false`). SHA-256 артефактов
`bytecode.object` (creation) и `deployedBytecode.object` (runtime) совпали для
всех четырёх production-контрактов:

| Контракт | creation SHA-256 | runtime SHA-256 |
|---|---|---|
| `Cr3dXGateway` | `df5252eee62f233f50dbf485808d73bfd392be05ceaaba7fc2468a33759f2257` | `8638148d0da48f9333d71354c19c573fd599ab7f870e4cc67424d7c08d270e76` |
| `Cr3dXVerifier` | `8afeb29895116ba3604e0532e4b2824ec38f6c116100fc44e968dfdc27d13f62` | `3cc800eb7903b5e92b4c7ea041907a7aeebd69037c72d790075cfdcac2882009` |
| `Cr3dXDeals` | `8177983c78ae6bb404aca259795ca597d943357a8e06aa1c5309ade0f8d0c943` | `98507d7c6acbaf4e8c7f8c7e5ac5acb01b19dff8ebc164af5bedf229c85715db` |
| `Cr3dXCredit` | `e79ab11b9a40108d242cee50816fc29819d661b8f64e3116f20e211270b6f8e7` | `1d99d42f9318274c742d49752e59dbe3a090ec310da13b53822cd9889e7b111d` |

Следствие, которое записывается явно: каталог `contracts/` больше не равен
байт-в-байт implementation-коммиту `1a30881` — `git diff 1a30881 -- contracts/`
показывает ровно один файл, `IBlockProver.sol`, и только строки комментария;
`test/` не изменён. Байткод, ABI и селекторы идентичны, задеплоенный комплект
v0.4.8 остаётся точным продуктом этого исходника. Это post-Phase-B source-only
clarification, а не новая редакция реализации.

**Английский дайджест спецификации.** Добавлен `docs/SPEC_DIGEST_EN.md`: ненормативный
английский пересказ разделов 3.3, 3.4, 4.3 и 4.4 (жизненный цикл сделки,
канонический итог как машина состояний, машина состояний доказательства с двумя
постоянными причинами и их приоритетом, применение фандинга и погашения в две фазы,
резерв, экспозиция, скор и лимит, обязательный сценарий INV-20). Инварианты
INV-1…INV-22 не пересказываются: дайджест ссылается на запечатанную английскую
`invariant-coverage-matrix.md` фазы A, где каждый инвариант уже записан как
свойство с оракулом. Шапка дайджеста прямо говорит, что авторитетна замороженная
русская редакция v0.4.8, а при расхождении дефектен дайджест. README получил ссылку
в шаге 2 contributor path и в списке ссылок. Спецификация и её запечатанный
входной пакет не менялись.

**Пин публичных адресов к леджеру.** В `scripts/lib/source-rules.test.ts` добавлен
блок `deployment anchors`: таблица deployed contracts в README и таблица current
live anchors в `docs/S5_LIVE_RUNBOOK.md` обязаны совпадать с
`deployments/sepolia.json` и `deployments/creditcoin.json` и не содержать ни одного
адреса из `previousVerifiers` / `previousDeals`. Известная коллизия nonce 0
(первый verifier на адресе гейта `0x11DD8a4c…a749`) учтена явно: это единственный
адрес, которому разрешено быть одновременно текущим и заменённым. Дашборд `ui/`
намеренно не прибит — он показывает замороженный снимок с привязкой к коммиту и
обязан сохранять provenance после будущего передеплоя. Мутационная проверка: замена
адреса Deals в README на заменённый `0x80a9…` и подмена строки Credit в ранбуке
ломают ровно по одному тесту. Число TypeScript-файлов с тестами не изменилось
(10), число тестов в них выросло с 86 до 89; Foundry-счётчик не затронут.

**Редакционное.** В `docs/WORKFLOW.md` упоминание конкретного имени локального файла
заменено на нейтральное «локальный файл инструкций, если он есть».

**Ветки.** Проверка через GitHub API показала, что на `origin` существуют только
`main` и `invariant-blind`: ветки PR #1, #2, #4, #5, #6 и двух прямых
audit-merge (`8ddebc3`, `e534346`) были удалены на сервере раньше, а в локальном
клоне оставались их непрочищенные remote-tracking ссылки. Выполнен `git fetch -p`,
пять локальных смерженных веток (PR #1, #3, #4, #5, #6) удалены.
`origin/invariant-blind` сохранена намеренно: она не смержена и содержит
единственную копию замороженной модели слепого агента для v0.4.2. История `main`
и subject merge-коммитов не переписывались, запечатанные артефакты не
редактировались.

Локальная проверка после правок: `forge build --force` чистый, кроме известного
косметического предупреждения `test/GateLog.t.sol:63` про `view` (кэшированная
сборка его не показывает); `forge test` 141/141, включая 8 invariant/property
suites; `npm run test:scripts` 10/10 файлов, 89 тестов; `npm run typecheck`,
`ui:typecheck`, `test:ui` 15/15, `ui:build` — чисто.
