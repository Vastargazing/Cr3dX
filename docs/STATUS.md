...
- поведение `applyEvidence` задано: ревертит ровно на неизвестном верификатору
  идентификаторе, всё остальное идемпотентно;
...
верификатора нет; всё остальное идемпотентно и возвращает состояние. Ответить
«ждёт» на непроверенный идентификатор было бы ложью: он ничего не ждёт, его нет.
