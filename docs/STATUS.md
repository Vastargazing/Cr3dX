...
Верхний раздел этой даты дополняет вывод: горизонт хранения checkpoint всё ещё
не установлен, поэтому здесь не утверждается бессрочная доказуемость старого факта.
